...
var.hot_streak_flamestrike = 0
var.hard_cast_flamestrike = 0
var.combustion_flamestrike = 0
var.arcane_explosion = 0
var.arcane_explosion_mana = 0
var.kindling_reduction = 0
var.skb_duration = 0
var.combustion_on_use = false
var.on_use_cutoff = 0
var.empyreal_ordnance_delay = 0
var.combustion_shifting_power = 0
...
actionList.combustion_phase = function()
    ...
    if (cast.able.pyroblast() and not isUnitCasting("player")) and buff.pyroclasm.react() and buff.pyroclasm.remains() > cast.time() then
        if cast.pyroblast(units.dyn40) then ui.debug("Casting Pyroblast 1 (Standard Rotation)") return true end 
    end
...
actionList.standard_rotation = function()
    ...
    if (cast.able.pyroblast() and not isUnitCasting("player")) and buff.pyroclasm.react() and cast.time.pyroblast() < buff.pyroclasm.remains() then
    ...
    if cast.able.fireBlast() and firestarterActive and not var.fire_blast_pooling and (((cast.current.fireball() and (cast.timeRemain() < 0.5 or not runeforge.firestorm) or cast.current.pyroblast() and (cast.timeRemain() < 0.5 or not runeforge.firestorm)) and buff.heatingUp.react()) or (talent.searingTouch and thp <= 30 and (buff.heatingUp.react() and not cast.current.scorch() or not buff.hotStreak.react() and not buff.heatingUp.react() and cast.current.scorch() and not (cast.inFlight.fireball() or cast.inFlight.pyroblast())))) then
    ...
    if cast.able.fireball() and not isUnitCasting("player") then
...
local function runRotation()
    ...
    ui                                            = br.player.ui
    cl                                            = br.read
    travelTime                                    = getDistance("target") / 50 --Ice lance
    ...
    var.phoenix_pooling = talent.runeOfPower and cd.runeOfPower.remain() < cd.phoenixFlames.remain() and cd.combustion.remain() > combustionROPcutoff and (cd.runeOfPower.remain() < ttd("target") or charges.runeOfPower.count() > 0) or cd.combustion.remain() < charges.phoenixFlames.timeTillFull() and cd.combustion.remain() < ttd("target")
